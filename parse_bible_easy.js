...
const outputFilePath = path.join(__dirname, 'bible_easy.json');
...
const main = () => {
    const bibleData = {
        old_testament_easy: processDirectory(path.join(assetDir, 'old_testament_easy')),
        new_testament_easy: processDirectory(path.join(assetDir, 'new_testament_easy'))
    };

    fs.writeFileSync(outputFilePath, JSON.stringify(bibleData, null, 2), 'utf-8');
    console.log('Successfully generated bible_easy.json');
};
